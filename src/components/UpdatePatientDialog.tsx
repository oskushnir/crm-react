...
export const UpdatePatientDialog: React.FC<Props> = ({ patientIdToEdit, patientDataToEdit }) => {
  ...
  const mutationUpdatePatient = useMutation({
    mutationFn: (data: { patientId: number } & PatientFormValues) =>
      updatePatient(data),
  });

  const mutationUpdateMedicalHistory = useMutation({
    mutationFn: (data: { patientId: number } & PatientFormMedicalValuesAPI) =>
      updatePatientMedicalHistory(data),
  });
  ...
  const handleNext = async (data: PatientFormValues) => {
    try {
      if (patientIdToEdit) {
        ...
        const isDataChanged = Object.keys(changedData).length > 0;
  ...
  const handleSubmit = async (data: PatientFormMedicalValues) => {
    try {
      const hasMedicalHistory = !!patientDataToEdit?.medicalHistory?.id;
      ...
      if (hasMedicalHistory) {
        ...
        if (isDataChanged) {
          await mutationUpdateMedicalHistory.mutateAsync({
            patientId: patientIdToEdit,
  ...
  const handleReset = () => {
    ...
    mutationUpdateMedicalHistory.reset();
  ...
  return (
    <DialogForAddClient
      ...
      description="Update the patient details and medical history."
      isEditMode={true}
        ...
        >
          <TabsContent value='patient'>
            <FormClient
              ...
              newPatientError={mutationUpdatePatient.isError}
              loading={mutationUpdatePatient.isPending}
            />
          </TabsContent>
          ...
          <TabsContent value='medicalHistory'>
            <FormMedicalClient
              ...
              error={mutationUpdateMedicalHistory.isError || mutationAddMedicalHistory.isError}
              loading={mutationUpdateMedicalHistory.isPending || mutationAddMedicalHistory.isPending}
            />
          </TabsContent>
        </TabsForm>
      }
